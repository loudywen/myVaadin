...
/*	
	...
	private Component buildTitle() {
		Label logo = new Label("Real-Time <strong>Dashboard</strong>", ContentMode.HTML);
	...
	private Component buildMenuItems() {
		...
		for (final DashboardViewType view : DashboardViewType.values()) {
			...
			/*
			 * if (view == DashboardViewType.REPORTS) { // Add drop target to
			 * reports button DragAndDropWrapper reports = new
			 * DragAndDropWrapper(menuItemComponent);
			 * reports.setSizeUndefined();
			 * reports.setDragStartMode(DragStartMode.NONE);
			 * reports.setDropHandler(new DropHandler() {
			 * 
			 * @Override public void drop(final DragAndDropEvent event) {
			 * UI.getCurrent().getNavigator().navigateTo(DashboardViewType.
			 * REPORTS.getViewName()); Table table = (Table)
			 * event.getTransferable().getSourceComponent();
			 * DashboardEventBus.post(new
			 * TransactionReportEvent((Collection<Transaction>)
			 * table.getValue())); }
			 * 
			 * @Override public AcceptCriterion getAcceptCriterion() { return
			 * AcceptItem.ALL; }
			 * 
			...
			if (view == DashboardViewType.DASHBOARD) {
				...
				notificationsBadge.setValue("1");
				notificationsBadge.setId(NOTIFICATIONS_BADGE_ID);
				menuItemComponent = buildBadgeWrapper(menuItemComponent, notificationsBadge);
			}
			/*
			 * if (view == DashboardViewType.REPORTS) { reportsBadge = new
			 * Label(); reportsBadge.setId(REPORTS_BADGE_ID); menuItemComponent
			 * = buildBadgeWrapper(menuItemComponent, reportsBadge); }
			 */
	 ...
	 */
	/*
	 * @Subscribe public void postViewChange(final PostViewChangeEvent event) {
	 * // After a successful view change the menu can be hidden in mobile view.
	 * getCompositionRoot().removeStyleName(STYLE_VISIBLE); }
	 */
	/*
	 * @Subscribe public void updateNotificationsCount( final
	...
	/*
	 * @Subscribe public void updateReportsCount(final ReportsCountUpdatedEvent
	...
	public final class ValoMenuItemButton extends Button {
		...
		/*
		 * @Subscribe public void postViewChange(final PostViewChangeEvent
		 * event) { removeStyleName(STYLE_SELECTED); if (event.getView() ==
